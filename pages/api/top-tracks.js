...
export default async function handler(req, res) {
  try {
    ...
    try {
      ...
      return res
        .status(500)
        .json({ message: 'Failed to fetch top tracks due to an internal error.' })
    }
    ...
    if (response === null) {
      ...
      return res
        .status(503)
        .json({ message: 'Could not retrieve top tracks from Spotify at this time.' })
    }
    ...
    if (!response.ok) {
      ...
      return res
        .status(statusCode)
        .json({ message: `Spotify API error: ${statusText}`, details: errorDetails })
    }
