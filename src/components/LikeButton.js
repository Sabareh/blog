...
export function LikeButton({ id, namespace = 'blog-post' }) {
  return (
    <LyketErrorBoundary>
      <LyketLikeButton id={id} namespace={namespace} component={LikeButton.templates.Simple} />
    </LyketErrorBoundary>
  )
}
...
LikeButton.templates = {
  Simple: ({ isLoading, isActive, handlePress, totalLikes }) => (
    <button
      onClick={handlePress}
      disabled={isLoading}
      className={`flex items-center gap-1 text-zinc-500 transition-colors hover:text-green-500 ${
