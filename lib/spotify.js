...
const getAccessToken = async () => {
  try {
    ...
    if (!response.ok) {
      ...
      console.error(`Spotify token error response: ${response.status} ${response.statusText}`, errorBody);
...
export const getNowPlaying = async () => {
  ...
  try {
    ...
    if (!response.ok) {
      const errorBody = await response.text();
      console.error(`Spotify Now Playing error response: ${response.status} ${response.statusText}`, errorBody);
