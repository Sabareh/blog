...
export default async function handler(req, res) { // Added req, res for explicit response handling
  try {
    // Fetch user data
    const userResponse = await fetch('https://api.github.com/users/sabare-ab', { cache: 'no-store' }); // Add cache control if needed
    if (!userResponse.ok) {
    ...
    // Fetch repos data
    const reposResponse = await fetch('https://api.github.com/users/sabare-ab/repos?per_page=100', { cache: 'no-store' }); // Add cache control
     if (!reposResponse.ok) {
        console.error(`GitHub repos fetch failed: ${reposResponse.status} ${reposResponse.statusText}`);
